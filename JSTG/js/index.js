...
//	elSeed.value = Math.round(Math.random()*10000);

	random.onclick = function() {
		elSeed.value = Math.round(Math.random()*10000);
		
		location.search = '?seed='+elSeed.value;
		generate.onclick();
	};
    ...
    generate.onclick = function(){
        var elTerDimension = document.getElementById('terdim'),
            ...
            elWater = document.getElementById("waterLimit"),
            elSand = document.getElementById("sandLimit"),
            elGrass = document.getElementById("grassLimit"),
            elMnt = document.getElementById("mntLimit"),
            elRock = document.getElementById("rockLimit");
        ...
        var water = parseInt(elWater.value, 10),
			sand = parseInt(elSand.value, 10),
			grass = parseInt(elGrass.value, 10),
			mnt = parseInt(elMnt.value, 10),
			rock = parseInt(elRock.value, 10);
		...
		sand = (sand ? sand : 35)/100;
		grass = (grass ? grass : 85)/100;
		mnt = (mnt ? mnt : 95)/100;
		rock = (rock ? rock : 100)/100;
...
//		console.log(map[256]);
    ...
    // Draw the map
    function drawMap(size, canvasId, mapData, mapType, waterLimit, sandLimit, grassLimit, mntLimit, rockLimit){
...
*/
	...
	for (var i = href.length; i > 0; --i) {
		var q = href[i-1].split('=');
